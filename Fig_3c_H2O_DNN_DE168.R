...
data <- data%>%select(Type:ZP, DE_168) ## Select feature and response variables
data <- data%>%filter(!is.na(ZP)) ## Eliminate the outlier
data <-data%>%filter(DE_168 > 0.01)
...
dummies     <-dummyVars(DE_168 ~., data = transformed)
mm          <-predict(dummies, newdata = transformed)

## "as.h2o": Create H2OFrame
MM<-as.h2o(mm %>% as.data.frame %>% mutate(Y=transformed[,"DE_168"]))
...
DL_DE_168_3  <- h2o.deeplearning(
    model_id             = "DL_DE_168_turned",
    ...
    hidden               = c(512,128,64),  ## Set up the layers of neural network
    ...
    score_validation_samples = 100000,     ## Number of validation set samples for scoring
...
## Save the model
model_path_DE_168 <- h2o.saveModel(object = DL_DE_168_3, 
                                   path = getwd(), 
                                   force = TRUE)
print(model_path_DE_168)

# load the model
# saved_model <- h2o.loadModel(model_path_DE_168)
...
# ## Prediction
# 
pred_train <- as.data.frame(h2o.predict(DL_DE_168_3, train[,c(predictors,response)]))%>%
    mutate(dtype="train", MAT=as.vector(splits_lable[[1]]$MAT))%>%
    rename(Pred = predict)

pred_test  <- as.data.frame(h2o.predict(DL_DE_168_3, valid[,c(predictors,response)]))%>%

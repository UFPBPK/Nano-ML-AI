...
data <- data%>%select(Type:ZP, DE_Tmax) ## Select feature and response variables
...
dummies     <-dummyVars(DE_Tmax ~., data = transformed)
mm          <-predict(dummies, newdata = transformed)

## "as.h2o": Create H2OFrame
MM<-as.h2o(mm %>% as.data.frame %>% mutate(Y=transformed[,"DE_Tmax"]))
...
DL_DE_TMAX_3  <- h2o.deeplearning(
    model_id             = "DL_DE_TMAX_tuned",
    ...
    hidden               = c(180,90,45),  ## Set up the layers of neural network
    ...
    #rho                  = 0.90,
    #epsilon              = 1e-8,    
    momentum_start       = 0.5,    ## manually tune momentum
    momentum_stable      = 0.9,
    momentum_ramp        = 1e6,
    l1                   = 1e-8,
    l2                   = 1e-8,
...
summary(DL_DE_TMAX_3)  ## Check results
plot(DL_DE_TMAX_3)     ## Plot the historical curve

## Save the model
model_path_DE_TMAX <- h2o.saveModel(object = DL_DE_TMAX_3, 
                                    path = getwd(), 
                                    force = TRUE)
print(model_path_DE_TMAX)
...
pred_train <- as.data.frame(h2o.predict(DL_DE_TMAX_3, train[,c(predictors,response)]))%>%
    mutate(dtype="train", MAT=as.vector(splits_lable[[1]]$MAT))%>%
    rename(Pred = predict)

pred_test  <- as.data.frame(h2o.predict(DL_DE_TMAX_3, valid[,c(predictors,response)]))%>%
